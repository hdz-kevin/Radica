...
Listing address fields (state, city, neighborhood, street_address, latitude, longitude) stay on listings. Do not extract a Location model/table. Do not store country, currency, or postal_code on listings or in the neighborhoods config. Display rent as MXN in the UI. Location is a value of the listing, not its own entity. A 1:1 split adds joins and N+1 on the catalog with no reuse. A shared places catalog can be additive later (place_id). Inertia may nest a location object when serializing; that is not a schema change.
