...
DatabaseSeeder creates 20 users (test@example.com plus 19 landlords) and 50 Teziutlán listings with 1–5 public-disk JPEGs so filter work has volume. Zone names are a private array in ListingSeeder, not a colonia catalog. Do not randomize ListingFactory::definition() (tests expect zone Centro and rent 12000); put realistic variety only in seeders.

## Seed listing photos from Unsplash fixtures
Listing seed photos come from committed Unsplash JPEGs in database/seeders/fixtures/listings/{apartment,room,house}/. Copy and recycle those files onto the public disk. Do not generate colored GD placeholders.
